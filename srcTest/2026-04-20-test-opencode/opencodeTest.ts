import {
    createAuthenticatedOpencodeSdkClient,
    createOpencodeAgentSessionId,
} from '../../src/routes/chatLlm/chatLlmAgent/utils/opencodeSdkHelpers';
...
async function main() {
    ...
    const client = await createAuthenticatedOpencodeSdkClient({
        ...
        basicAuth:
            basicAuthPassword.trim().length >= 1
                ? { username: basicAuthUsername || 'opencode', password: basicAuthPassword }
                : undefined,
    });
    ...
    while (Date.now() - start < timeoutMs) {
        const listRes = await client.file.list({
            query: { path: '/output', directory: workspaceDirectory },
        } as any);
        const files = (listRes as any)?.data;
        if (Array.isArray(files)) {
            const pdf = files.find(
                (f: any) => typeof f?.name === 'string' && f.name.toLowerCase().endsWith('.pdf')
            );
            if (pdf) {
...
main().catch((e) => {
    // eslint-disable-next-line no-console
    console.error('❌ opencodeTest failed:', e instanceof Error ? e.message : e);
    process.exitCode = 1;
});
...
// npx ts-node -r dotenv/config ./srcTest/2026-04-20-test-opencode/opencodeTest.ts