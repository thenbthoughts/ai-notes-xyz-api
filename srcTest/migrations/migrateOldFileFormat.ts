...
const migrateOldFileFormat = async () => {
    try {
        ...
        // Find all chatLlm records with old format fileUrl (starts with /)
        const oldFormatRecords = await ModelChatLlm.find({
            fileUrl: { $ne: '' },
            // threadId: new mongoose.Types.ObjectId('693585d1e530582e1b875419'),
        });
        console.log('oldFormatRecords: ', oldFormatRecords);
        ...
        for (const record of oldFormatRecords) {
            try {
                ...
                const parentEntityId = record.threadId?.toString() || '';
                ...
                // Update chatLlm record with new fileUrl
                await ModelChatLlm.updateOne(
                    { _id: record._id },
                    { $set: { fileUrl: objectKey } }
                );

                console.log(`Updated chatLlm record with new fileUrl`);
...
// npx ts-node -r dotenv/config ./srcTest/migrations/migrateOldFileFormat.ts