...
const s3FileIndexSchema = new Schema<IS3FileIndex>({
    ...
    originalParentPath: { type: String, default: '' },
    versionHistory: { type: [{ content: String, savedAt: Date, size: Number }], default: [] },
});
...
s3FileIndexSchema.index({ userId: 1, bucketName: 1, isTrashed: 1 });
s3FileIndexSchema.index({ textContent: 'text', fileName: 'text' });
