...
const loadThreadSessionId = async (
    ...
    const thread = await ModelChatLlmThread.findById(threadId)
        .select('opencodeSessionId threadTitle aiModelProvider aiModelName opencodeMcpEnabled opencodeMaxAnswerTimeMinutes')
    ...
    const opencodeMaxAnswerTimeMinutes =
        thread && typeof thread.opencodeMaxAnswerTimeMinutes === 'number' && (thread.opencodeMaxAnswerTimeMinutes as number) > 0
            ? (thread.opencodeMaxAnswerTimeMinutes as number)
            : 60;
    return { sessionId, sessionTitle, aiModelProvider, aiModelName, opencodeMcpEnabled, opencodeMaxAnswerTimeMinutes };
};
...
export const agentOpencodeRunPipeline = async (
    ...
    try {
        ...
        const { sessionId: existingSessionId, sessionTitle, aiModelProvider, aiModelName, opencodeMcpEnabled, opencodeMaxAnswerTimeMinutes } =
        ...
        const called = await agentOpencodeStepCall({
            ...
            sessionTitle,
            maxAnswerTimeMinutes: opencodeMaxAnswerTimeMinutes,
            libraryCounts,
            previousAnswerText,
        });
