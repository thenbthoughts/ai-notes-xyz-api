...
export const agentOpencodeWorkspacePaths = ({
    ...
    // Minimal layout: keep only CHAT.md + ANSWER.md (plus uploads/opencode.json/.env).
    // No input-* / output-* per-thread files, no INSTRUCTION.md — prompt goes via API, history is CHAT.md.
    // ANSWER.md is cleared on each new user message and written with the final answer.
    const agentWorkspaceDir = root;
    return {
        root,
        agentWorkspaceDir,
        inputPrompt: `${root}/ANSWER.md`,
        outputPrompt: `${root}/ANSWER.md`,
        chatHistory: `${root}/CHAT.md`,
...
/** Copy remote thread-root files to a local folder (fixtures, prior outputs). */
...
/** Upload a local directory into the remote thread-root folder. */
