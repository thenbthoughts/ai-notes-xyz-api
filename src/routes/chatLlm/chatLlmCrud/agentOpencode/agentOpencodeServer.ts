...
import { AGENT_OPENCODE_MAX_ANSWER_TIME_MS } from './agentOpencodeConstants';
...
export const opencodeGetSessionMessagesViaShell = async (params: {
    shell: AgentOpencodeShellConfig;
    directory: string;
    sessionId: string;
    timeoutMs?: number;
}): Promise<{ messages: Array<{ info: unknown; parts: unknown[] }>; raw: unknown }> => {
    const sid = String(params.sessionId || '').trim();
    if (!isOpencodeSessionId(sid)) return { messages: [], raw: null };
    const query = params.directory ? `?directory=${encodeURIComponent(params.directory)}` : '';
    const { status, bodyJson } = await runCurlViaShell({
        shell: params.shell,
        method: 'GET',
    ...
    if (status < 200 || status >= 300) return { messages: [], raw: bodyJson };
    if (Array.isArray(bodyJson)) return { messages: bodyJson as Array<{ info: unknown; parts: unknown[] }>, raw: bodyJson };
    return { messages: [], raw: bodyJson };
};

export const opencodeExportSessionViaShell = async (params: {
    shell: AgentOpencodeShellConfig;
    directory: string;
    sessionId: string;
    timeoutMs?: number;
}): Promise<{ exportText: string; raw: unknown }> => {
    const { messages, raw } = await opencodeGetSessionMessagesViaShell(params);
    if (messages.length === 0) return { exportText: '', raw };
    const lines: string[] = [];
    for (const msg of messages) {
        const info = msg.info as Record<string, unknown> | undefined;
        const role = typeof info?.role === 'string' ? String(info.role) : (msg as Record<string, unknown>).role as string || 'assistant';
        ...
        for (const p of parts) {
            ...
            else if (typeof (part as Record<string, unknown>).content === 'string' && String((part as Record<string, unknown>).content).trim()) textParts.push(String((part as Record<string, unknown>).content));
        }
        const text = textParts.join('\n\n').trim();
        if (!text) continue;
        const header = role.toLowerCase().includes('user') ? '## User' : '## Assistant';
        lines.push(`${header}\n\n${text}`);
    }
...
const runCurlViaShell = async (params: {
    ...
    const timeoutMs = Math.min(Math.max(params.timeoutMs ?? 300_000, 1), AGENT_OPENCODE_MAX_ANSWER_TIME_MS);
...
export const opencodeCreateSessionViaShell = async (params: {
    ...
    if (status < 200 || status >= 300) {
        const obj = bodyJson && typeof bodyJson === 'object' ? (bodyJson as Record<string, unknown>) : null;
        const dataObj = obj && typeof obj.data === 'object' ? (obj.data as Record<string, unknown>) : null;
        const msg =
            (obj && typeof obj.message === 'string' ? String(obj.message) : '') ||
            (dataObj && typeof dataObj.message === 'string' ? String(dataObj.message) : '') ||
            (typeof obj?.name === 'string' ? String(obj.name) : '') ||
            (typeof bodyJson === 'string' ? bodyJson : '') ||
            bodyText ||
            `Create session HTTP ${status}`;
        const ref = dataObj && typeof dataObj.ref === 'string' ? String(dataObj.ref) : (obj && typeof (obj as Record<string, unknown>).ref === 'string' ? String((obj as Record<string, unknown>).ref) : '');
        const full = ref ? `${msg} (ref:${ref}) [directory:${directory}]` : `${msg} [directory:${directory}]`;
        throw new Error(String(full).slice(0, 1200) || `Create session failed HTTP ${status}`);
    }
...
export const opencodePromptSessionViaShell = async (params: {
    ...
    if (status < 200 || status >= 300) {
        const obj = bodyJson && typeof bodyJson === 'object' ? (bodyJson as Record<string, unknown>) : null;
        const dataObj = obj && typeof obj.data === 'object' ? (obj.data as Record<string, unknown>) : null;
        const nameVal = obj && typeof obj.name === 'string' ? String(obj.name) : '';
        const msgVal =
            (obj && typeof obj.message === 'string' ? String(obj.message) : '') ||
            (dataObj && typeof dataObj.message === 'string' ? String(dataObj.message) : '') ||
            (typeof bodyJson === 'string' ? String(bodyJson) : '') ||
            bodyText ||
            `Prompt HTTP ${status}`;
        const ref = dataObj && typeof dataObj.ref === 'string' ? String(dataObj.ref) : '';
        const full = `${nameVal ? nameVal + ': ' : ''}${msgVal}${ref ? ` (ref:${ref})` : ''} [model:${model.providerID}/${model.modelID} session:${sid} directory:${directory}]`;
        throw new Error(String(full).slice(0, 1500) || `Prompt failed HTTP ${status}`);
    }
