...
router.post('/notesGet', middlewareUserAuth, async (req: Request, res: Response) => {
    try {
        ...
        if (typeof req.body?.search === 'string' && req.body.search.trim().length >= 1) {
            const raw = req.body.search.trim().slice(0, 120);
            ...
            stateDocument.push(regexStage);
            stateCount.push(regexStage);
        }
