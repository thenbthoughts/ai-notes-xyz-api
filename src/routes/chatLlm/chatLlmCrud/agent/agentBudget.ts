...
export const formatAgentBudgetContext = (status: AgentBudgetStatus): Record<string, unknown> => ({
    ...
    instruction: status.maxExceeded
        ? 'Budget max reached (tokens and/or iterations). Prefer readyToSynthesize=true immediately with best available evidence.'
        : !status.minsMet
          ? 'Minimum budget not met yet. Do NOT set readyToSynthesize=true until both token and iteration mins are met, unless max is imminent.'
          : status.nearMax
            ? 'Near max budget. Prefer synthesizing soon; avoid expensive exploratory steps.'
            : 'Budget OK. Continue toward a grounded final answer; synthesize when evidence is enough and mins are met.',
});
