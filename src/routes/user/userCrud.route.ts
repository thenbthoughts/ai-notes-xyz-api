...
import { putFile } from '../../utils/upload/uploadFunc';
...
router.post('/userAvatarUpload', middlewareUserAuth, fileUpload({ limits: { fileSize: 5 * 1024 * 1024 }, abortOnLimit: true }), async (req: Request, res: Response) => {
    try {
        const started = Date.now();
        if (!req.files || !req.files.avatar) {
            return res.status(400).json({ message: 'avatar file is required (field avatar)' });
        }
        const uploaded = req.files.avatar as fileUpload.UploadedFile;
        if (Array.isArray(uploaded)) {
            return res.status(400).json({ message: 'Upload a single file' });
        }
        ...
        const ext = uploaded.name.split('.').pop() || 'png';
        const fileName = `avatar-${res.locals.auth_userId}-${Date.now()}.${ext}`;
        const result = await putFile({ fileName, fileContent: uploaded.data, contentType: uploaded.mimetype || 'image/png', storageType: 'gridfs' });
        if (!result.success || !result.fileId) {
            return res.status(500).json({ message: result.error || 'Upload failed' });
        }
        await ModelUser.findByIdAndUpdate(res.locals.auth_userId, { $set: { profilePictureLink: result.fileId } });
        const latencyMs = Date.now() - started;
        return res.json({ success: true, fileId: result.fileId, profilePictureLink: result.fileId, latencyMs });
    } catch (error) {
        console.error(error);
        return res.status(500).json({ message: 'Server error' });
    }
...
router.get('/providerHealthPing', middlewareUserAuth, async (req: Request, res: Response) => {
    try {
        const started = Date.now();
        const doc = await ModelUserApiKey.findOne({ userId: res.locals.auth_userId }).lean();
        const latencyMs = Date.now() - started;
        if (!doc) {
            return res.json({ latencyMs, valid: {} });
        }
        return res.json({ latencyMs, valid: { apiKeyGroqValid: !!doc.apiKeyGroqValid, apiKeyOpenrouterValid: !!doc.apiKeyOpenrouterValid, apiKeyS3Valid: !!doc.apiKeyS3Valid, apiKeyOllamaValid: !!doc.apiKeyOllamaValid, apiKeyLocalaiValid: !!doc.apiKeyLocalaiValid, apiKeyOpenaiValid: !!doc.apiKeyOpenaiValid, smtpValid: !!doc.smtpValid, telegramValid: !!doc.telegramValid } });
    } catch (error) {
        console.error(error);
        return res.status(500).json({ message: 'Server error' });
    }
