...
router.post('/testS3Connection', middlewareUserAuth, async (req: Request, res: Response) => {
    try {
        const started = Date.now();
        const s3Valid = await ModelUserApiKey.findOne({ userId: res.locals.auth_userId }).lean();
        const latencyMs = Date.now() - started;
        if (!s3Valid || !s3Valid.apiKeyS3Valid) return res.status(400).json({ message: 'S3 not configured', latencyMs });
        return res.json({ success: true, latencyMs, valid: true });
    } catch (error) {
        console.error(error);
        return res.status(500).json({ message: 'Server error' });
    }
